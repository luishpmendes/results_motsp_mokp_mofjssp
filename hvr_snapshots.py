...
# Get all unique time values from hvr_snapshots_df
time_values = hvr_snapshots_df['snapshot time'].unique()
time_values.sort()

num_instances = len(hvr_snapshots_df['instance'].unique())
num_seeds = len(hvr_snapshots_df['seed'].unique())

# Calculate the cumulative distribution for each solver
for solver in solvers:
    print(solver)
    hvr_snapshots_solver_df = hvr_snapshots_df[hvr_snapshots_df['solver'] == solver]
    for time in time_values:
        # For each time, calculate the proportion of executions where the solver's performance is at least the target value
        hvr_snapshots_solver_time_df = hvr_snapshots_solver_df[hvr_snapshots_solver_df['snapshot time'] <= time]
        percentage_meeting_target = 100 * np.mean(hvr_snapshots_solver_time_df['metric value'] >= target_value) if len(hvr_snapshots_solver_time_df) > num_instances * num_seeds else 0
